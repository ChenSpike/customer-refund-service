...
import { getCase, decideCase } from '../api';
...
export default function CaseDetail({ traceId, onBack, onChanged }) {
  ...
  const act = async (action, amount) => {
    setSubmitting(true);
    try {
      await decideCase(traceId, action, 'You', undefined, amount);
      setPartialAmount('');
      load();
      onChanged && onChanged();
    } catch (err) {
  ...
  const actPartialRefund = () => {
    const amount = Number(partialAmount);
    if (!amount || amount <= 0) {
      alert('Enter a partial refund amount greater than 0.');
      return;
    }
    act('partial_refund', amount);
  };
  ...
  const hasPendingApproval = !!detail.pendingApprovalId;
  const isQuarantine = detail.status === 'quarantined';
  const isNeedsInfo = detail.status === 'needs_info';
  const isResolved = detail.status === 'auto_approved' || detail.status === 'rejected';
    ...
    <div style={{ padding: '28px 40px 60px', maxWidth: 1360 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 22, marginTop: 24, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18, minWidth: 0 }}>
          ...
          {isReview && (
            <div style={card}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14 }}>Admin Action</div>
              {!hasPendingApproval && (
                <div style={{ fontSize: 12.5, color: colors.textMuted, marginBottom: 14 }}>
                  Awaiting Policy Agent evaluation. Nothing to act on yet.
                </div>
              )}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <ActionButton disabled={submitting || !hasPendingApproval} onClick={() => act('approve')} variant="approve">Approve Refund</ActionButton>
                <div style={{ display: 'flex', gap: 8 }}>
                  <input
                    ...
                    style={{
                      flex: 1, minWidth: 0, padding: '10px 12px', borderRadius: 8, fontSize: 13,
                      border: `1px solid ${colors.borderStrong}`, background: 'transparent',
                    }}
                  />
                  <ActionButton disabled={submitting || !hasPendingApproval} onClick={actPartialRefund} variant="neutral">Partial Refund</ActionButton>
                </div>
                <ActionButton disabled={submitting || !hasPendingApproval} onClick={() => act('request_info')} variant="neutral">Request Info</ActionButton>
                <ActionButton disabled={submitting || !hasPendingApproval} onClick={() => act('deny')} variant="reject">Deny</ActionButton>
              </div>
            </div>
          )}
          {isQuarantine && (
            <div style={card}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14 }}>Admin Action</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <ActionButton disabled={submitting} onClick={() => act('confirm_block')} variant="reject-solid">Confirm Block</ActionButton>
                <ActionButton disabled={submitting} onClick={() => act('release')} variant="neutral">Override &amp; Route to Manual Review</ActionButton>
              </div>
            </div>
          )}
          {isNeedsInfo && (
...
function ActionButton({ children, variant, ...props }) {
  const styles = {
    approve: { background: colors.good, color: 'oklch(0.99 0.01 150)', border: 'none', fontWeight: 600 },
    'reject-solid': { background: colors.danger, color: 'oklch(0.99 0.01 25)', border: 'none', fontWeight: 600 },
    reject: { background: 'transparent', color: 'oklch(0.45 0.19 25)', border: '1px solid oklch(0.5 0.19 25 / 0.35)' },
    neutral: { background: 'transparent', color: 'oklch(0.3 0.02 260)', border: `1px solid ${colors.borderStrong}` },
  }[variant];
  return (
    <button {...props} style={{
      padding: 10, borderRadius: 8, fontSize: 13, cursor: props.disabled ? 'default' : 'pointer',
      opacity: props.disabled ? 0.6 : 1, ...styles,
    }}>{children}</button>
  );
}
