...
import { colors, card, pageWrap, STATUS_META, filterTabStyle, money } from '../theme';

const FILTERS = [
  { key: 'all', label: 'All', hint: 'Every case in the queue, regardless of status.' },
  { key: 'pending', label: 'Pending Review', hint: 'Still being evaluated by the Policy Agent, or waiting on a human’s manual-review decision.' },
...
export default function Overview({ cases, loading, filter, setFilter, previewTraceId, setPreviewTraceId, onSelectCase, onGoNav }) {
  const counts = useMemo(() => ({
    ...
    pending: cases.filter((c) => c.status === 'pending_review' || c.status === 'manual_review').length,
  ...
  const filteredCases = useMemo(() => {
    ...
    if (filter === 'pending') return cases.filter((c) => c.status === 'pending_review' || c.status === 'manual_review');
  ...
  const overviewStats = [
    { label: 'Open Cases', value: String(counts.all - counts.auto_approved - counts.human_approved - counts.rejected), detail: 'Currently moving through the pipeline' },
    { label: 'Pending Review', value: String(counts.pending), detail: 'Awaiting human sign-off' },
    { label: 'Quarantined', value: String(counts.quarantined), detail: 'Blocked by the governance layer' },
    { label: 'Auto-Approved', value: String(counts.auto_approved), detail: 'Resolved without human intervention' },
    { label: 'Automation Rate', value: counts.all ? `${Math.round((counts.auto_approved / counts.all) * 100)}%` : '0%', detail: 'Resolved before reaching a human' },
  ];
    ...
    <div style={{ ...pageWrap, maxWidth: 1180 }}>
      ...
      <div style={{ border: `1px solid ${colors.border}`, borderRadius: 10, overflowX: 'auto' }}>
        <div style={{
          ...
          <div>Ticket</div><div>Customer / Request</div><div>Reason</div><div>Amount</div><div>Status</div><div>Risk</div><div>Updated</div>
        </div>
        ...
        {filteredCases.map((c) => (
          ...
          >
            ...
            <div style={{ fontSize: 13, fontFamily: 'ui-monospace,monospace' }}>${money(c.amount)}</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
              <div>
                ...
                <div style={{ fontSize: 12.5, color: colors.textMuted, marginTop: 8 }}>
                  {previewCase.customer} · {previewCase.reasonLabel} · <span style={{ fontFamily: 'ui-monospace,monospace' }}>${money(previewCase.amount)}</span>
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 32 }}>
        <button onClick={() => onGoNav('governance')} style={quickLinkStyle}>
          ...
          <div style={{ fontSize: 12, color: colors.textMuted, marginTop: 4 }}>Review flagged security incidents</div>
        </button>
