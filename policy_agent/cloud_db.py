...
MIGRATION_001_PATH = POLICY_AGENT_DIR / "migrations" / "001_policy_governance_separation.sql"
MIGRATION_002_PATH = POLICY_AGENT_DIR / "migrations" / "002_unified_human_approval_trigger.sql"
...
class GCPRepository:
    ...
    def migrate_schema(self) -> dict[str, bool]:
        separation = _migration_statements(MIGRATION_001_PATH, expected=3)
        unified_trigger = _migration_statements(MIGRATION_002_PATH, expected=4)
    ...
    def check_schema(self) -> dict[str, int]:
        ...
        try:
            ...
            nullable_triggers = [name for name, nullable in cursor.fetchall() if nullable != "NO"]
            ...
            return {
                "source_handoffs": int(benchmark_sources),
    ...
    def reset_policy_agent_data(self) -> dict[str, int]:
        """Return benchmark workflows to their triage -> policy_agent baseline."""
        ...
        try:
            ...
            cursor.execute(
                """
                DELETE reviews
                FROM policy_review_events reviews
            ...
            counts["policy_review_events"] = cursor.rowcount
            cursor.execute(
                """
                DELETE events
                FROM governance_events events
        ...
        try:
            ...
            queries = {
                ...
                "human_approvals": """
                    ...
                    WHERE approval_id LIKE 'POL-APP-%'
                      AND trace_id REGEXP '^TRACE-POL-(00[1-9]|01[0-9]|020)$'
        ...
        try:
            ...
            cursor.execute(
                ...
                WHERE p.from_agent = 'policy_agent'
                  AND p.trace_id REGEXP '^TRACE-POL-(00[1-9]|01[0-9]|020)$'
                GROUP BY
                  p.handoff_id, p.trace_id, p.ticket_id, p.to_agent,
                  p.input_json, p.output_json, p.input_tokens, p.output_tokens,
    ...
    def fetch_review_records(self) -> dict[str, list[dict[str, Any]]]:
        ...
        try:
            ...
            return {"policy": policy, "governance": governance, "approvals": approvals}
        finally:
            connection.close()
    ...
    def integrity_counts(self) -> dict[str, int]:
        ...
        try:
            cursor = connection.cursor()
            result: dict[str, int] = {}
            for table in ("audit_log", "governance_events", "policy_review_events", "human_approvals"):
                cursor.execute(
                    f"""
            ...
            cursor.execute(
                ...
                WHERE (approvals.triggering_event_type = 'governance' AND governance.event_id IS NULL)
    ...
    def _persist_policy_review(self, cursor: Any, output: PolicyAgentOutput) -> str | None:
        ...
        review_policies = [
            policy.policy_id
            for policy in output.policy_evaluation.matched_policies
            if policy.effect == "requires_review"
        ]
        if not review_policies:
            review_policies = [policy.policy_id for policy in output.policy_evaluation.matched_policies]
        evidence = {
            "matched_policies": [
                policy.model_dump(mode="json") for policy in output.policy_evaluation.matched_policies
            ],
                    ...
                    json.dumps(
                        {
                            ...
                            "governance": output.governance.model_dump(mode="json"),
                        },
                        ensure_ascii=False,
                    ),
    ...
    def _persist_human_approval(
        ...
        approval_id = existing_approval_id or self._next_prefixed_id(
            cursor,
            "human_approvals",
            "approval_id",
            "POL-APP-",
        )
        trigger_type, trigger_id = _approval_trigger(
            output.case.trace_id,
            policy_event_id,
            governance_event_ids,
        )
...
def _approval_trigger(
    trace_id: str,
    policy_event_id: str | None,
    governance_event_ids: list[str],
) -> tuple[Literal["governance", "policy_review"], str]:
    if governance_event_ids:
        return "governance", governance_event_ids[0]
    if policy_event_id:
        return "policy_review", policy_event_id
    raise CloudDatabaseError(
        f"{trace_id}: human approval route requires a governance or policy review event"
    )
